...
function Mode2() {
  ...
  const getCountryCode = (input) => {
    ...
    if (cleanInput.length === 2 && countries.isValid(cleanInput.toUpperCase())) {
      return cleanInput.toLowerCase();
    }

    // 2. Full Name (Pakistan -> pk)
    const code = countries.getAlpha2Code(cleanInput, "en");
    if (code) return code.toLowerCase();

    // 3. Emoji Flag
    const flagRegex = /[\uD83C][\uDDE6-\uDDFF][\uD83C][\uDDE6-\uDDFF]/;
    const match = cleanInput.match(flagRegex);
    if (match) {
      const codeFromEmoji = Array.from(match[0])
        .map(char => String.fromCharCode(char.codePointAt(0) - 127397))
        .join('')
        .toLowerCase();
      return codeFromEmoji;
    }
  ...
  const updateSnakeData = useCallback((data) => {
    ...
    // Yahan country detection lagayi hai jo 195 countries handle karegi
    const rawInput = data.countryCode || data.message || "";
    const countryCode = getCountryCode(rawInput); 

    const snakeKey = data.username.toLowerCase().trim();
    ...
    setSnakes((prev) => {
      const existingSnake = prev[snakeKey];
      return {
        ...prev,
        [snakeKey]: { 
          ...data, 
    ...
    socketRef.current.on("newCommentsBatch", (batch) => {
      if (Array.isArray(batch)) {
        batch.forEach((data, index) => {
          setTimeout(() => updateSnakeData(data), index * 150);
        });
      }
    });

    socketRef.current.on("newComment", (data) => updateSnakeData(data));
  ...
  return (
    <div className="flex h-screen items-center justify-center overflow-hidden font-sans bg-zinc-950">
                  ...
                  <AnimatePresence>
                    {showMenu && (
                      <motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} className="absolute top-12 left-0 w-48 bg-[#0f0f0f] border border-white/10 rounded-2xl overflow-hidden shadow-2xl z-[800]">
                        ...
                        <button onClick={() => navigate('/')} className="w-full px-4 py-3.5 flex items-center gap-3 text-white text-[11px] font-bold hover:bg-white/5 transition-colors group">
